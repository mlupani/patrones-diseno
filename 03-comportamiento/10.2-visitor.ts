...
class MaintenanceCostVisitor implements Visitor {
  ...
  visitCar(car: Car): void {
    ...
    const cost = car.getKilometers() * 0.1 + (car.getYear()) * 50
  ...
  visitMotorcycle(motorcycle: Motorcycle): void {
    const cost = motorcycle.getKilometers() * 0.05 + (motorcycle.getYear()) * 30
  ...
  visitTruck(truck: Truck): void {
    const cost = truck.getKilometers() * 0.15 + truck.getLoadCapacity() * 20 + (truck.getYear()) * 100
