...
class ControlTower {
  ...
  registerAirplane(airplane: Airplane):void {
    if(!this.airplanes.find(airplane => airplane))
  ...
  sendMessage(sender: Airplane, message: string): void {
    console.log(
      `\n\n\n%c${sender} envía: %c${message} `,
      COLORS.blue,
      COLORS.white
    );

    const airplandsToSend = this.airplanes.filter((airplane) => airplane !== sender);
    for (const airplane of airplandsToSend) {
...
class Airplane {
  ...
  receiveMessage(sender: Airplane, message: string): void {
    ...
    this.controlTower.sendMessage(this, message);
  }
}
